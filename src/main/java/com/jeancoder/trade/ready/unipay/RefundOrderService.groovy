...
import com.milepai.core.utils.web.XmlHandlerTool

class RefundOrderService {

	JCLogger LOGGER = LoggerSource.getLogger(RefundOrderService.class.getName());
	
	static final RefundOrderService _instance_ = new RefundOrderService();
	
	TradeService trade_service = TradeService.INSTANCE();
	
	static RefundOrderService INSTANCE() {
		return _instance_;
	}
	 ...
	 * @return
	 */
	def refund_pay_router(ProjectGeneralConfig config, def ct, TradeInfo trade, TradeOrder order, TradePayInfo pay_info, Map<String, String> other_param) {
		...
		if(ct=='201101') {
			...
			return refund_wx(config, trade, order, pay_info, other_param);
		} else if(ct=='201001') {
			...
			//会员卡被扫
			return refund_mc(config, trade, order, pay_info, other_param);
		} else if(ct=='001001') {
			other_param.put("ctcode", "001001");
			//现金支付
			pay_result = refund_cash(config, trade, order, pay_info, other_param);
		} else if(ct=='202001') {
			//支付宝被扫退款
			other_param.put('ctcode', '202001');
			pay_result = refund_alipay(config, trade, order, pay_info, other_param);
		}
		return pay_result;
	}
	
	def refund_wx(ProjectGeneralConfig config, TradeInfo trade, TradeOrder order, TradePayInfo pay_info, Map<String, String> other_param) {
		PayResult pay_result = new PayResult();
		...
		data.put('refund_fee', order.pay_amount.setScale(0).toString());
		...
		try {
			...
			if(return_code=='SUCCESS'&&result_code=='SUCCESS') {
				...
				trade_service.save_trade_order_refund_data(trade, order, [pay_result, pay_data_map]);
				return pay_result;
			}
	...
	def refund_cash(ProjectGeneralConfig config, TradeInfo trade, TradeOrder order, TradePayInfo pay_info, Map<String, String> other_param) {
		PayResult pay_result = new PayResult();
		...
		pay_data_map['trans_pay_amount'] = order.pay_amount.setScale(0);
	...
	def refund_mc(ProjectGeneralConfig config, TradeInfo trade, TradeOrder order, TradePayInfo pay_info, Map<String, String> other_param) {
		PayResult pay_result = new PayResult();
		...
		def param_dic = [pid:pid,num:num,oc:order.oc,on:order.order_num];
		
		TradeResult param = JC.internal.call(TradeResult, 'crm', '/api/order/refund', param_dic);
	...
	def refund_alipay(ProjectGeneralConfig config, TradeInfo trade, TradeOrder order, TradePayInfo pay_info, Map<String, String> other_param) {
		...
		def out_trade_refund_no = trade.tnum;
		
		body['out_trade_no'] = out_trade_refund_no;
		body['refund_amount'] = order.pay_amount.divide(new BigDecimal('100')).setScale(2, BigDecimal.ROUND_HALF_UP).toString();	//订单退款金额
		...
		LOGGER.info('alipay_refund_order_data:' + ret_data);
