...
import com.jeancoder.trade.ready.printer.TicketContentHtml
...
def mpv = JC.internal.param('mpv')?.trim();
def mpc = JC.internal.param('mpc')?.trim();
...
for(x in trade_orders) {
	if(x.oc=='1000') {
		...
		if(coupon_arr&&coupon_arr.get('2000')!=null) {
			def coupon_ids = coupon_arr.get('2000');
			SimpleAjax result_price = JC.internal.call(SimpleAjax, 'ticketingsys', '/incall/order/preferential_reserve', [o_id:x.order_id,unicode:coupon_ids,pref:'200',op:'use',pid:pid]);
			if (!StringUtil.isEmpty(coupon_ids)) {
				if (result_price == null)  {
					return SimpleAjax.notAvailable("卡劵模块通讯异常")
				}
		...
		if(unicode) {
			...
			LOGGER.info('RESERVE_5000 RESULT ::: ' + JackSonBeanMapper.toJson(result_price));
			queried_orders.add(result_price);
			if(result_price&&result_price.available&&mc_data==null) {
				mc_data = result_price.data;
			}
...
for(x in trade_orders) {
	if(x.oc=='1000') {
		...
		smarttemplate = URLEncoder.encode(smarttemplate, 'UTF-8');
		smarttemplate = URLEncoder.encode(smarttemplate, 'UTF-8');
		PrinterDto pricedto = PrinterUtil.getPrinterDtoLsit(pid,ccid, x.oc, smarttemplate);
		...
		//打印电影票
		x = TradeService.INSTANCE().getTradeById(x.id)
		String default_smarttemplate = new FeedPrinter().get_printer_content_for_ticket(trade, x);				//获得默认打印票务模版
		
		PrinterDto printer_dto_s = PrinterUtil.getPrinterDtoLsit(pid,ccid, x.oc, default_smarttemplate);	//获取收银台对应的配置信息
		
		smarttemplate = PrinterUtil.fill_ticket_printer_dto(trade, x, printer_dto_s);
...
LOGGER.info 'ct pub_result ret_data=' + JackSonBeanMapper.toJson(ret_data);
return ret_data;
