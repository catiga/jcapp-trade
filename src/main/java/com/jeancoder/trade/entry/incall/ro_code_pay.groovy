...
def o_rem = JC.request.param('o_rem')?.trim();
...
def total_amount = trade.pay_amount;
def trade_use_prefs = [];
if(queried_orders) {
	//这里处理会员卡和优惠券
	def index = 0;
...
if(trade.total_amount.compareTo(new BigDecimal("0"))<=0||trade.pay_amount.compareTo(new BigDecimal("0"))<=0) {
	//0元的交易直接归到现金支付里面
	ct = '001001';
}
